...
func main() {
	...
	http.HandleFunc("/list", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(id) == 0 {
			...
			postcode, postcodeErr := strconv.Atoi(r.URL.Query().Get("postcode"))
			isUrgent, urgentErr := strconv.ParseBool(r.URL.Query().Get("isUrgent"))
			for id, msg := range msgs {
				if postcodeErr == nil && postcode != msg.Postcode {
					continue
				}
				if urgentErr == nil && isUrgent != msg.IsUrgent {
					continue
				}
